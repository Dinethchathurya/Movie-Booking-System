...
export const signup = async (req, res) => {
  const { firstName,lastName, email,gender,nic,address,mobile, password } = req.body;
  if (!firstName || !email || !password) {
    return res.status(422).json({ message: "Fields are required." });
  }
  const hashedPassword = bcryptjs.hashSync(password, 10);
  const newUser = new User({ firstName, lastName, email, gender, nic, address, mobile, password: hashedPassword });
  try {
    await newUser.save();
