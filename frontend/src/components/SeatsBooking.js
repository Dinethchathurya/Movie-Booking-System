      ...
      <div className="flex flex-col lg:flex-row text-white mt-14 mb-14 gap-2">
        ...
        <div className="mb-4">
            <p className="text-white font-bold">
              Booking available from: <span className="text-blue-600">{startDate}</span> to{" "}
              <span className="text-blue-600">{endDate}</span>
            </p>
          </div>
          <div className="mb-4 flex gap-4">
            <div className="flex-1">
              <label className="block text-black font-bold mb-2">Select Date:</label>
              <select
  ...
  {availableDates.map((date) => (
    <option key={date.toISOString()} value={date.toISOString().split("T")[0]}>
      {date.toLocaleDateString()}
            ...
            <div className="flex-1">
              <label className="block text-black font-bold mb-2 ">Select Time:</label>
        ...
        <PayPalScriptProvider options={{ "client-id": "Acg6e5yPBwhLoNPSodGeED-W8Zc1OnHAohnoMWGPELwRC2_KRsU-ANFJNlARz4bESiYh4Ljn7xj1_P33" }}>
...
totalCost > 0 &&
selectedDate &&
JSON.stringify(selectedSeats) === JSON.stringify([...selectedSeats]) &&
...
totalCost === selectedSeats.length * pricePerSeat ? (
  ...
  <p>
    Please ensure all data is selected, valid, and consistent (e.g., seats, total cost, and selections match).
  </p>
)}
